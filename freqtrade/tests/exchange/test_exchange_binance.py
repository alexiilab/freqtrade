...
# external package Binance.
# By inserting a mock, we redirect those
# calls.
# The faked binance API is called just 'fakebin'
# The freqtrade.exchange.Binance is a
# wrapper, and is called 'realbin'
...
class FakeBinance():
    def __init__(self, success=True):
        ...
        binance._API = MagicMock()
        binance._API.order_limit_buy = self.fake_buysell_limit
        binance._API.order_limit_sell = self.fake_buysell_limit
        binance._API.get_asset_balance = self.fake_get_asset_balance
        binance._API.get_account = self.fake_get_account
        binance._API.get_ticker = self.fake_get_ticker
        binance._API.get_order = self.fake_get_order
        binance._API.cancel_order = self.fake_cancel_order
        binance._API.get_exchange_info = self.fake_get_exchange_info
        binance._API.get_klines = self.fake_get_klines
...
#todo
    ...
    def fake_get_asset_balance(self, cur):
        return {'success': self.success,
                'result': {'Balance': 1234},
                'message': 'unbalanced'}
    ...
    def fake_get_klines(self, pair, interval):
        return self.result or {'success': self.success,
